...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s• ًٍُِّdec_ZORO ༆ • %s/%s > OK %s > CP %s > %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
